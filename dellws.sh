...
clear
NUMBER_OF_CLIENTS=$(grep -c -E "^#vmtls# " "/usr/local/etc/xray/config.json")
	...
	grep -E "^#vmtls# " "/usr/local/etc/xray/config.json" | cut -d ' ' -f 2-3 | nl -s ') '
...
user=$(grep -E "^#vmtls# " "/usr/local/etc/xray/config.json" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^#vmtls# " "/usr/local/etc/xray/config.json" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
